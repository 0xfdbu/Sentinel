...
/**
 * @title CCIPTransferConsumerWithACEFixed
 * @notice FIXED version - CRE Consumer for cross-chain transfers
 * @dev This version allows bankOperator to trigger CCIP directly for workflow compatibility
 */
contract CCIPTransferConsumerWithACEFixed {
    // Report instructions
    ...
    event CCIPProcessedViaBankOperator(bytes32 indexed reportHash, address indexed receiver, uint256 amount, uint64 destinationChain);
    ...
    error CCIPTransferFailed(string reason);
    ...
    /**
     * @notice Process CCIP transfer via bank operator (for CRE workflow compatibility)
     * @dev This allows the bank operator to trigger CCIP directly without going through Forwarder
     * @param receiver Address to receive tokens on destination chain
     * @param amount Amount of tokens to transfer
     * @param destinationChain CCIP chain selector
     */
    function processCCIPViaBankOperator(
        address receiver,
        uint256 amount,
        uint64 destinationChain
    ) external onlyBankOperator whenNotPaused returns (bytes32) {
        ...
        emit CCIPProcessedViaBankOperator(reportHash, receiver, amount, destinationChain);
        
        // Process the transfer with this contract as sender
        return _processCCIPTransfer(reportHash, address(this), receiver, amount, destinationChain);
    }
