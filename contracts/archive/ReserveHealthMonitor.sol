 ...
 * @custom:track Chainlink Convergence Hackathon 2026 - Risk & Compliance
 * 
 * Monitors:
...
contract ReserveHealthMonitor is Ownable, ReentrancyGuard {
    ...
    constructor(address _registry, address _guardian) Ownable() {
    ...
    function batchUpdateHealthMetrics(
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            if (monitoringConfigs[contracts[i]].isActive) {
                this.updateHealthMetrics(contracts[i], tvls[i], collateralRatios[i]);
            }
        }
    }
