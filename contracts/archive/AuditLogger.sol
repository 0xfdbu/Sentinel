...
contract AuditLogger is Ownable {
    ...
    function logScan(
        ...
        return _logScanInternal(target, vulnHash, severity, metadata);
    }
    
    /**
     * @notice Simplified logScan (no metadata)
     */
    function logScanSimple(
        address target,
        bytes32 vulnHash,
        uint8 severity
    ) external onlyScanner returns (uint256) {
        return _logScanInternal(target, vulnHash, severity, "");
    }
    
    /**
     * @notice Internal implementation of logScan
     */
    function _logScanInternal(
        address target,
        bytes32 vulnHash,
        uint8 severity,
