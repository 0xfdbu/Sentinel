/**
 * USDA Stablecoin Deployment for Arbitrum Sepolia
 * 
 * Deploys ACE-compliant stablecoin with CCIP support
 * 
 * Usage: npx hardhat run deploy/DeployUSDAArbitrumSepolia.ts --network arbitrumSepolia
 */
...
const CCIP_ROUTERS = {
  arbitrumSepolia: "0x2a9c5afb0d0e4bab2bcdae109ec4b0c4be15a165", // lowercase
};
...
async function main() {
  ...
  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log("║     USDA STABLECOIN DEPLOYMENT - ARBITRUM SEPOLIA          ║");
  console.log("╚════════════════════════════════════════════════════════════╝");
  console.log();
  ...
  const ccipRouter = CCIP_ROUTERS.arbitrumSepolia;
  console.log("Using Arbitrum Sepolia CCIP Router:", ccipRouter);
  console.log();
  ...
  const deploymentInfo = {
    ...
    crossChain: {
      ...
      arbitrumSepoliaChainSelector: CHAIN_SELECTORS.arbitrumSepolia.toString(),
    },
    aceEnabled: true,
  };

  const deploymentsPath = join(__dirname, "../config/deployments");
  const filename = `usda-stablecoin-arbitrum-sepolia-${Date.now()}.json`;
  writeFileSync(join(deploymentsPath, filename), JSON.stringify(deploymentInfo, null, 2));
  writeFileSync(join(deploymentsPath, "usda-stablecoin-arbitrum-sepolia-latest.json"), JSON.stringify(deploymentInfo, null, 2));

  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log("║         ARBITRUM SEPOLIA DEPLOYMENT COMPLETE               ║");
  console.log("╚════════════════════════════════════════════════════════════╝");
  console.log();
  console.log("USDA STABLECOIN (ARBITRUM SEPOLIA):");
  console.log("─────────────────────────────────────");
  ...
  console.log("Arbitrum Sepolia USDA:", stablecoinAddress);
  ...
  console.log();
  console.log("NEXT STEP: Link Sepolia to Arbitrum Sepolia");
  console.log(`  ARBITRUM_SEPOLIA_USDA=${stablecoinAddress} npx hardhat run deploy/LinkSepoliaToArbitrum.ts --network sepolia`);
}
