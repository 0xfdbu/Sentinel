...
export default function Visualizer() {
  ...
  useEffect(() => {
    ...
    monitoredContracts.forEach((contract, index) => {
      ...
      if (!existing) {
        ...
        if (contract.isPaused) existing.color = COLORS.contract.paused
      }
    })
  ...
  useEffect(() => {
    ...
    // CRE Node (right)
    const cre = nodesRef.current.get('cre-node')
    if (!cre) {
      nodesRef.current.set('cre-node', {
        id: 'cre-node',
        type: 'cre',
        label: 'CRE',
        x: w * 0.88,
    ...
    const animate = () => {
      ...
      nodes.forEach(node => {
        ...
        if (showLabels) {
          ...
          const typeText = node.type === 'sentinel' ? 'NODE' : 
                          node.type === 'cre' ? 'CRE' : 
  ...
  return (
    <div className="h-[calc(100vh-80px)] md:h-[calc(100vh-100px)] bg-neutral-950 flex flex-col overflow-hidden">
      ...
      <div ref={containerRef} className="flex-1 relative">
        ...
        {/* Legend */}
        <div className="absolute bottom-6 left-6 bg-slate-900/95 backdrop-blur rounded-xl p-4 border border-slate-800">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <div className="w-4 h-4 rounded-full bg-violet-500/30 border-2 border-violet-500" />
              <span className="text-sm text-neutral-400">Chainlink CRE</span>
            </div>
            <div className="flex items-center gap-3">
        ...
        {selectedNode && (
          <motion.div
            ...
            className="absolute bottom-6 right-6 w-72 bg-slate-900/95 backdrop-blur rounded-xl p-4 border border-slate-800"
          >
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-300 font-mono text-xs">{selectedNode.id.slice(0, 20)}...</span>
              </div>
