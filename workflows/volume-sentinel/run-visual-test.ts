...
function log(title: string, content: any, color = colors.cyan) {
...
async function fetchFinnhubNews() {
  ...
  try {
    ...
    const allNews = response.data.slice(0, 20);
    
    // Filter for crypto-related news
    const cryptoNews = allNews.filter((item: any) => {
      const text = `${item.headline} ${item.summary}`.toLowerCase();
      return keywords.some(kw => text.includes(kw));
    });
    
    console.log(`${colors.green}✓ Retrieved ${allNews.length} news items${colors.reset}`);
    ...
    cryptoNews.slice(0, 5).forEach((item: any, i: number) => {
...
function analyzeSentiment(text: string): number {
  const positive = ['surge', 'rally', 'gain', 'bull', 'adopt', 'breakthrough', 'high', 'up', 'growth', 'success'];
  const negative = ['crash', 'hack', 'fall', 'bear', 'ban', 'regulation', 'low', 'down', 'loss', 'risk', 'fraud'];
...
async function fetchCoinGeckoData() {
  ...
  try {
    ...
    const btc = pricesResponse.data.find((c: any) => c.id === 'bitcoin');
    const eth = pricesResponse.data.find((c: any) => c.id === 'ethereum');
...
function calculateSentiment(news: any[], marketData: any) {
  arrow('Processing');
...
async function analyzeWithGrok(sentiment: number, marketData: any) {
  arrow('AI Inference');
...
function simulateBlockchainWrite(recommendation: any, sentiment: number) {
