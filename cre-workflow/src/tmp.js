...
var fetchContractSource = (sendRequester, contractAddress, chainId) => {
  const response = sendRequester.sendRequest({
    request: {
      url: chainId === 11155111 ? `https://api-sepolia.etherscan.io/api?module=contract&action=getsourcecode&address=${contractAddress}` : `https://api.etherscan.io/v2/api?chainid=${chainId}&module=contract&action=getsourcecode&address=${contractAddress}`,
      method: "GET",
      multiHeaders: {}
    },
    vaultDonSecrets: [
...
var analyzeWithGrok = (sendRequester, sourceCode) => {
...
var onCronTrigger = (runtime2) => {
  ...
  const contractAddress = runtime2.config.contracts?.pausableVault || "0xc7CD6F13A4bE91604BCc04A78f57531d30808D1C";
  const chainId = runtime2.config.chainId || 11155111;
  runtime2.log(`Scanning contract: ${contractAddress}`);
  try {
    runtime2.log("[1/3] Fetching contract source...");
    const sourceResult = confHTTPClient.sendRequest(runtime2, (sendRequester) => fetchContractSource(sendRequester, contractAddress, chainId), consensusIdenticalAggregation())(runtime2.config).result();
    const sourceText = new TextDecoder().decode(base64ToBytes2(sourceResult.bodyBase64));
    const sourceJson = JSON.parse(sourceText);
    const sourceCode = sourceJson.result?.[0]?.SourceCode || "";
    if (!sourceCode) {
      runtime2.log(`⚠️ No source code for ${contractAddress}`);
      return JSON.stringify({ error: "No source code", contract: contractAddress });
    }
    runtime2.log(`✓ Source fetched (${sourceCode.length} chars)`);
    runtime2.log("[2/3] Analyzing with xAI Grok...");
    const analysis = confHTTPClient.sendRequest(runtime2, (sendRequester) => analyzeWithGrok(sendRequester, sourceCode), consensusIdenticalAggregation())(runtime2.config).result();
    runtime2.log(`✓ Analysis: ${analysis.severity} (${Math.round(analysis.confidence * 100)}% confidence)`);
    runtime2.log("[3/3] Evaluating risk...");
    const { action, vulnHash } = evaluateRisk(analysis);
    runtime2.log(`✓ Action: ${action}`);
    if (action === "PAUSE") {
      runtime2.log(`\uD83D\uDEA8 CRITICAL: Emergency pause recommended!`);
    }
    const result = {
      contractAddress,
      chainId,
    ...
    const output = JSON.stringify({
      ...
      environment: runtime2.config.environment,
      result
    });
    runtime2.log("--- Scan Complete ---");
    return output;
  } catch (error) {
